...
class EnhancedBulkOverviewGenerator:
    """Handles bulk generation of college overviews with start-from capability."""
    ...
    async def generate_overview_for_college(self, college_id: int, max_retries: int = 3) -> bool:
        """Generate overview for a single college with AI score retry mechanism."""
        try:
            ...
            while attempts < max_retries:
                ...
                logger.info(f"🔄 {attempt_prefix} Generating content for college {college_id}")
                ...
                logger.info(f"✅ Successfully generated overview for college {college_id} {attempt_prefix}")
            ...
            logger.info(f"📊 Using content for college {college_id} with final AI score: {final_score_pct}%")
...
async def main() -> int:
    ...
    bulk_generator = EnhancedBulkOverviewGenerator(
        config=config,
